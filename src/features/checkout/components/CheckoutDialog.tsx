...
export default function CheckoutDialog({
  ...
  /*
   * =========================================================
   * PLAY & EARN WALLET CHECKOUT
   * =========================================================
   *
   * Completely separate from the existing T&M Wallet.
   * These states/holds use only the Play & Earn wallet tables
   * and RPCs.
   */

  const [
    playEarnWalletBalancePaise,
    setPlayEarnWalletBalancePaise,
  ] = useState(0);

  const [
    playEarnWalletEligibility,
    setPlayEarnWalletEligibility,
  ] = useState<{
    wallet_enabled: boolean;
    ...
    daily_remaining_paise: number | null;
    monthly_remaining_paise: number | null;
  ...
  const [
    playEarnWalletLoading,
    setPlayEarnWalletLoading,
  ] = useState(false);

  const [
    playEarnWalletApplying,
    setPlayEarnWalletApplying,
  ] = useState(false);

  const [
    playEarnWalletSelected,
    setPlayEarnWalletSelected,
  ] = useState(false);

  const [
    playEarnWalletHoldId,
    setPlayEarnWalletHoldId,
  ] = useState<string | null>(null);

  const [
    playEarnWalletAmountPaise,
    setPlayEarnWalletAmountPaise,
  ] = useState(0);

  const [
    playEarnWalletError,
    setPlayEarnWalletError,
  ] = useState("");
  ...
  /*
   * =========================================================
   * LOAD PLAY & EARN WALLET CHECKOUT ELIGIBILITY
   * =========================================================
   *
   * This never calls or modifies the regular T&M Wallet RPCs.
   */
  ...
  useEffect(() => {
    ...
    async function loadPlayEarnWallet() {
      ...
      if (
        ...
        if (mounted) {
          setPlayEarnWalletEligibility(null);
          setPlayEarnWalletBalancePaise(0);
        }
        return;
      }
      ...
      try {
        ...
        setPlayEarnWalletLoading(true);
        setPlayEarnWalletError("");

        const {
          data: walletData,
          error: walletErrorResult,
        } = await supabase.rpc(
          "get_or_create_my_play_earn_wallet"
        );

        if (walletErrorResult) {
          throw walletErrorResult;
        }
        ...
        const {
          ...
          "get_play_earn_wallet_checkout_eligibility",
          {
            p_customer_id:
              customer.id,
        ...
        setPlayEarnWalletBalancePaise(
          Math.max(
            ...
            Number(
              ...
              eligibility?.wallet_balance_paise ??
              0
            )
          )
        );
            ...
            ? {
                ...
                wallet_balance_paise:
                  Math.max(
                    0,
                    Number(
                      eligibility.wallet_balance_paise || 0
                    )
                  ),
                ...
                minimum_order_value_paise:
                  Math.max(
                    0,
                    Number(
                      eligibility.minimum_order_value_paise || 0
                    )
                  ),
                ...
                minimum_wallet_balance_paise:
                  Math.max(
                    0,
                    Number(
                      eligibility.minimum_wallet_balance_paise || 0
                    )
                  ),

                maximum_redemption_paise:
                  eligibility.maximum_redemption_paise == null
                    ? null
                    : Math.max(
                        0,
                        Number(
                          eligibility.maximum_redemption_paise
                        )
                      ),
                ...
                maximum_redemption_percentage:
                  Math.max(
                    0,
                    Number(
                      eligibility.maximum_redemption_percentage || 0
                    )
                  ),

                daily_remaining_paise:
                  eligibility.daily_remaining_paise == null
                    ? null
                    : Math.max(
                        0,
                        Number(
                          eligibility.daily_remaining_paise
                        )
                      ),

                monthly_remaining_paise:
                  eligibility.monthly_remaining_paise == null
                    ? null
                    : Math.max(
                        0,
                        Number(
                          eligibility.monthly_remaining_paise
                        )
                      ),

                allow_on_sale_products:
                  Boolean(
                    eligibility.allow_on_sale_products
                  ),

                allow_on_discounted_products:
                  Boolean(
                    eligibility.allow_on_discounted_products
                  ),

                allow_with_coupon:
                  Boolean(
                    eligibility.allow_with_coupon
                  ),

                allow_with_regular_wallet:
                  Boolean(
                    eligibility.allow_with_regular_wallet
                  ),

                allow_shipping_charges:
                  Boolean(
                    eligibility.allow_shipping_charges
                  ),

                allow_cod:
                  Boolean(
                    eligibility.allow_cod
                  ),

                allow_online_payment:
                  Boolean(
                    eligibility.allow_online_payment
                  ),

                has_coupon:
                  Boolean(
                    eligibility.has_coupon
                  ),
        ...
        if (
          ...
          setPlayEarnWalletHoldId(null);
          setPlayEarnWalletAmountPaise(0);
          setPlayEarnWalletSelected(false);
        }
        ...
        setPlayEarnWalletError(
          "Play & Earn Wallet is temporarily unavailable."
        );

      } finally {

        if (mounted) {
          setPlayEarnWalletLoading(false);
        }
  ...
  /*
   * =========================================================
   * PLAY & EARN WALLET HOLD
   * =========================================================
   */

  async function releasePlayEarnWalletHold() {

    if (!playEarnWalletHoldId) {
      return true;
    }
    ...
    try {
      ...
      const {
        ...
        "release_play_earn_wallet_checkout_hold",
        {
          p_hold_id:
            playEarnWalletHoldId,
        }
      ...
      console.warn(
        "Play & Earn checkout hold release failed:",
        error
      );
  ...
  async function createPlayEarnWalletHoldForQuote(
    quoteId: string,
    totalAmount: number
  ) {
    ...
    try {
      ...
      setPlayEarnWalletApplying(true);
      setPlayEarnWalletError("");

      /*
       * Re-fetch the authoritative eligibility for this exact
       * finalized quote. This is especially important when the
       * customer goes back to Address and receives a new quote.
       */
      const {
        data: eligibilityData,
        error: eligibilityError,
      ...
      if (!eligibility?.wallet_enabled) {
        setPlayEarnWalletHoldId(null);
        setPlayEarnWalletAmountPaise(0);
        setPlayEarnWalletSelected(false);
        setPlayEarnWalletError(
          "Play & Earn Wallet cannot be used on this order."
        );
        return false;
      }

      /*
       * Default checkout settings do not allow combining the
       * Play & Earn Wallet with the regular T&M Wallet.
       * Respect the Admin setting if it is changed later.
       */
      if (
        walletSelected &&
        !Boolean(
        ...
        setPlayEarnWalletSelected(false);
        setPlayEarnWalletError(
          "Play & Earn Wallet cannot be combined with the regular T&M Wallet."
        );
        return false;
      }
      ...
      if (requestedAmountPaise <= 0) {
        ...
        setPlayEarnWalletError(
          Number(
            ...
            ? eligibility.has_coupon &&
              !eligibility.allow_with_coupon
              ? "Play & Earn Wallet cannot be used with this coupon."
              : "Play & Earn Wallet cannot be used on the eligible items in this order."
            : "You don't have enough Play & Earn Wallet balance."
        );
      ...
      const {
        ...
        "create_play_earn_wallet_checkout_hold",
        {
          p_customer_id:
            customer.id,
      ...
      const holdId =
        hold?.hold_id ??
        hold?.id ??
        hold?.play_earn_wallet_hold_id;

      const returnedAmountPaise =
        Number(
          hold?.amount_paise ??
          hold?.play_earn_wallet_amount_paise ??
          requestedAmountPaise
        );
      ...
      if (
        ...
        console.error(
          "Unexpected Play & Earn wallet hold response:",
          data
        );

        throw new Error(
          "Play & Earn Wallet could not be reserved for this checkout."
        );
      }

      setPlayEarnWalletHoldId(holdId);
      setPlayEarnWalletAmountPaise(
        returnedAmountPaise
      );
      setPlayEarnWalletSelected(true);
      ...
      console.error(
        "Play & Earn wallet checkout hold creation failed:",
        error
      );
      ...
      setPlayEarnWalletError(
        error?.message ||
        "We couldn't apply Play & Earn Wallet right now. Please try again."
      );
      ...
      setPlayEarnWalletApplying(false);
  ...
  async function togglePlayEarnWallet() {

    if (playEarnWalletApplying) {
      return;
    }

    if (playEarnWalletSelected) {

      setPlayEarnWalletApplying(true);

      try {
        await releasePlayEarnWalletHold();
        setPlayEarnWalletError("");
      } finally {
        setPlayEarnWalletApplying(false);
      }
    ...
    if (
      walletSelected &&
      !Boolean(
        playEarnWalletEligibility?.allow_with_regular_wallet
      )
    ) {
      setPlayEarnWalletError(
        "Play & Earn Wallet cannot be combined with the regular T&M Wallet."
    ...
    await createPlayEarnWalletHoldForQuote(
      checkoutQuoteId,
      verifiedCheckoutPricing?.totalAmount ??
        finalAmount
    );
  ...
  async function handleAddressContinue(
    ...
    const shouldReapplyPlayEarnWallet =
      playEarnWalletSelected;
    ...
    if (playEarnWalletHoldId) {
      ...
      if (!released) {
        ...
        setShippingError(
          "We couldn't refresh your Play & Earn Wallet reservation. Please try again."
        );
    ...
    } else if (shouldReapplyPlayEarnWallet) {
...

      {
        ...
        onSuccess: async (
          ...
          try {
            ...
            if (shouldReapplyPlayEarnWallet) {
    ...
    try {
      ...
      const result =
        await createOrder({
          ...
          /*
           * Play & Earn Wallet is intentionally separate from the
           * existing T&M Wallet fields above.
           */
          playEarnWalletHoldId:
  payment.playEarnWalletHoldId ??
  recoverySnapshot?.playEarnWalletHoldId ??
  playEarnWalletHoldId ??
  null,

playEarnWalletAmountPaise:
  Number(
    payment.playEarnWalletAmountPaise ??
    recoverySnapshot?.playEarnWalletAmountPaise ??
    playEarnWalletAmountPaise ??
    0
  ),
            ...
            !isAddingNewAddress && (
              ...
              >
                ...
                >
                  ...
                  {/* GIFT WRAP */}
                  {step !== "payment" && giftWrapSettings?.enabled && (
                      ...
                      <label className="flex cursor-pointer items-center justify-between gap-3">
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            if (isBuyNow) {
                              setBuyNowGiftWrapSelected(checked);
                            } else {
            ...
            step === "payment" && (
              ...
              <>
...

                {
                  !isGuestCheckoutCustomer &&
                  customer?.id &&
                  playEarnWalletEligibility?.wallet_enabled &&
                  (
                    Number(
                      playEarnWalletEligibility?.maximum_wallet_paise || 0
                    ) > 0
                  ) && (
                    ...
                    <div
                      className={`
                        ...
                        transition-all
                        duration-300
                        motion-safe:animate-[fadeUp_450ms_ease-out]
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              >
                                {
                                  playEarnWalletLoading
                                    ? "Checking your Play & Earn balance..."
                                    : `Balance: ₹${(
                                        Number(
                                          playEarnWalletBalancePaise || 0
                                        ) / 100
                                      ).toLocaleString("en-IN", {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2,
                          ...
                          <button
                            ...
                            aria-label={
                              playEarnWalletSelected
                                ? "Remove Play & Earn Wallet credit"
                                : "Use Play & Earn Wallet credit"
                            }
                          >
                            ...
                            >
                              ...
                              >
                                {Math.max(
                                  ...
                                  <span className="font-medium text-green-700">
                                    ✓ Your Play &amp; Earn Wallet covers the full order total
                                  </span>
                                )}
                              </div>
                        ...
                        {!playEarnWalletSelected &&
                          ...
                          playEarnWalletBalancePaise > 0 && (
                            <button
                              type="button"
                              onClick={togglePlayEarnWallet}
                              disabled={playEarnWalletApplying}
                              className="
                                relative
                                mt-3
                                flex
                            ...
                            >
                              ...
                              <span
                                ...
                                "
                              >
                                Use your game rewards on this order
                              </span>
                        ...
                        {walletSelected &&
                          !Boolean(
                            ...
                            <div
                              ...
                              "
                            >
                              Play &amp; Earn Wallet can't be combined with T&amp;M Wallet under the current checkout settings.
                            </div>
                          )
                        }
